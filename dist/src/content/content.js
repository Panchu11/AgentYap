...
Rewritten reply:`;return await b(e)}catch(e){console.error("Error rewriting reply:",e);const t=["Absolutely! ","This! ","Exactly! ","So true! ","Facts! "];return t[Math.floor(Math.random()*t.length)]+s.replace(/^(Great|Amazing|Awesome|Nice|Cool)/,"").trim()}}class C{constructor(){d(this,"observer",null);d(this,"isContextValid",!0);d(this,"injectedButtons",new Set);this.init()}init(){var e;if(!((e=chrome==null?void 0:chrome.runtime)!=null&&e.id)){console.warn("Extension context invalid, stopping initialization");return}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>this.startObserving()):this.startObserving(),chrome.runtime.onMessage.addListener((t,a,r)=>(this.handleMessage(t,r),!0)),chrome.runtime.onConnect.addListener(()=>{this.isContextValid=!0})}async handleMessage(e,t){var a,r,n;if(!this.isContextValid||!((a=chrome==null?void 0:chrome.runtime)!=null&&a.id)){t({success:!1,error:"Extension context invalidated. Please refresh the page."});return}try{switch(e.type){case"GENERATE_REPLY":try{const i=await y(e.tweetText,e.tone);try{(r=chrome==null?void 0:chrome.runtime)!=null&&r.id&&chrome.runtime.sendMessage({type:"REPLY_GENERATED",reply:i})}catch(o){console.warn("Could not send message to popup:",o)}t({success:!0,reply:i})}catch(i){console.error("Error generating reply:",i);const o=i instanceof Error?i.message:"Unknown error occurred";t({success:!1,error:o})}break;case"REWRITE_REPLY":try{const i=await p(e.originalReply);try{(n=chrome==null?void 0:chrome.runtime)!=null&&n.id&&chrome.runtime.sendMessage({type:"REPLY_GENERATED",reply:i})}catch(o){console.warn("Could not send message to popup:",o)}t({success:!0,reply:i})}catch(i){console.error("Error rewriting reply:",i);const o=i instanceof Error?i.message:"Unknown error occurred";t({success:!1,error:o})}break}}catch(i){console.error("Error in handleMessage:",i),t({success:!1,error:"Message handling failed"})}}startObserving(){this.isContextValid&&(this.observer=new MutationObserver(e=>{if(!this.isContextValid)return;e.forEach(t=>{t.type==="childList"&&t.addedNodes.length>0&&setTimeout(()=>this.checkForReplyBoxes(),300)})}),this.observer.observe(document.body,{childList:!0,subtree:!0}),this.checkForReplyBoxes())}checkForReplyBoxes(){this.cleanupOrphanedButtons();const e=["[data-testid=\"tweetTextarea_0\"]","[data-testid=\"tweetTextarea_1\"]","[role=\"textbox\"][data-testid*=\"tweet\"]",".public-DraftEditor-content","[contenteditable=\"true\"][data-testid*=\"tweet\"]"];for(const t of e){const a=document.querySelectorAll(t);a.forEach(r=>{this.maybeInjectFloatingButton(r)})}}cleanupOrphanedButtons(){const e=document.querySelectorAll(".yapmate-floating-btn");e.forEach(t=>{const a=t.getAttribute("data-reply-box-id");a&&!document.querySelector(`[data-yapmate-id="${a}"]`)&&(t.remove(),this.injectedButtons.delete(a))})}maybeInjectFloatingButton(e){const t=this.createUniqueId(e);if(this.injectedButtons.has(t)||!this.isReplyBox(e))return;const a=this.findTweetTextForReply(e);a&&(e.setAttribute("data-yapmate-id",t),this.createFloatingButton(e,a,t),this.injectedButtons.add(t))}createUniqueId(e){const t=e.getBoundingClientRect(),a=Date.now(),r=Math.random().toString(36).substr(2,5);return`yapmate-${Math.round(t.top)}-${Math.round(t.left)}-${a}-${r}`}isReplyBox(e){const t=["[data-testid*=\"reply\"]","[aria-label*=\"reply\"]","[aria-label*=\"Reply\"]"];let a=e.parentElement,r=0;for(;a&&r<10;){for(const i of t)if(a.querySelector(i)||a.matches(i))return!0;a=a.parentElement,r++}if(e.closest("[role=\"dialog\"]"))return!0;const n=this.findTweetTextForReply(e);return!!(n&&n.length>10)}createFloatingButton(e,t,a){const r=document.createElement("button");r.className="yapmate-floating-btn",r.setAttribute("data-reply-box-id",a),r.innerHTML="🤖 AI";const n=e.getBoundingClientRect();r.style.cssText=`
      position: fixed;
      top: ${n.top+window.scrollY-5}px;
      right: ${window.innerWidth-n.right+window.scrollX+10}px;
      width: 40px;
      height: 30px;
    ...
    `,r.addEventListener("mouseenter",()=>{r.style.transform="scale(1.1)",r.style.boxShadow="0 4px 12px rgba(29, 161, 242, 0.4)"}),r.addEventListener("mouseleave",()=>{r.style.transform="scale(1)",r.style.boxShadow="0 2px 8px rgba(29, 161, 242, 0.3)"}),r.addEventListener("click",async i=>{var o;i.preventDefault(),i.stopPropagation(),!this.isContextValid||!((o=chrome==null?void 0:chrome.runtime)!=null&&o.id)?this.showError(r,"Extension Error"):this.showTonePopup(r,e,t)});const c=()=>{const i=e.getBoundingClientRect();if(i.width===0&&i.height===0){r.remove(),this.injectedButtons.delete(a);return}r.style.top=`${i.top+window.scrollY-5}px`,r.style.right=`${window.innerWidth-i.right+window.scrollX+10}px`};window.addEventListener("scroll",c,{passive:!0}),window.addEventListener("resize",c,{passive:!0}),r.setAttribute("data-cleanup","true"),document.body.appendChild(r)}showTonePopup(e,t,a){const r=document.querySelector(".yapmate-tone-popup");r&&r.remove();const n=document.createElement("div");n.className="yapmate-tone-popup";const i=e.getBoundingClientRect();n.style.cssText=`
      position: fixed;
      top: ${i.bottom+5}px;
      ...
      padding: 8px;
      min-width: 120px;
    `,[{value:"Smart",emoji:"🧠",label:"Smart"},{value:"Funny",emoji:"😂",label:"Funny"},{value:"Serious",emoji:"💼",label:"Serious"},{value:"Degen",emoji:"🚀",label:"Degen"}].forEach(c=>{const l=document.createElement("button");l.innerHTML=`${c.emoji} ${c.label}`,l.style.cssText=`
        width: 100%;
        padding: 6px 8px;
        border: none;
      ...
      `,l.addEventListener("mouseenter",()=>{l.style.background="#f3f4f6"}),l.addEventListener("mouseleave",()=>{l.style.background="transparent"}),l.addEventListener("click",async()=>{n.remove(),await this.generateAndFillReply(e,t,a,c.value)}),n.appendChild(l)});const o=c=>{!n.contains(c.target)&&!e.contains(c.target)&&(n.remove(),document.removeEventListener("click",o))};setTimeout(()=>{document.addEventListener("click",o)},100),document.body.appendChild(n)}async generateAndFillReply(e,t,a,r){e.innerHTML="⏳",e.style.background="#0d8bd9";try{const n=await y(a,r);await this.fillReplyBox(t,n),e.innerHTML="✅",e.style.background="#10b981",this.addRewriteButton(e,t,n),setTimeout(()=>{e.innerHTML="🤖 AI",e.style.background="linear-gradient(135deg, #1da1f2, #0d8bd9)"},2e3)}catch(n){console.error("Error generating reply:",n),this.showError(e,"Error")}}addRewriteButton(e,t,a){const r=document.querySelector(".yapmate-rewrite-btn");r&&r.remove();const n=document.createElement("button");n.className="yapmate-rewrite-btn",n.innerHTML="🔄";const i=e.getBoundingClientRect();n.style.cssText=`
      position: fixed;
      top: ${i.top}px;
      right: ${window.innerWidth-i.left+10}px;
      width: 30px;
      height: 30px;
      background: #6b7280;
      color: white;
      ...
      align-items: center;
      justify-content: center;
    `,n.addEventListener("click",async()=>{n.innerHTML="⏳",n.style.background="#4b5563";try{const o=await p(a);await this.fillReplyBox(t,o),n.innerHTML="✅",n.style.background="#10b981",a=o,setTimeout(()=>{n.innerHTML="🔄",n.style.background="#6b7280"},1500)}catch(o){console.error("Error rewriting reply:",o),n.innerHTML="❌",n.style.background="#ef4444",setTimeout(()=>{n.innerHTML="🔄",n.style.background="#6b7280"},2e3)}}),document.body.appendChild(n),setTimeout(()=>{n.remove()},1e4)}showError(e,t){e.innerHTML="❌",e.style.background="#ef4444",setTimeout(()=>{e.innerHTML="🤖 AI",e.style.background="linear-gradient(135deg, #1da1f2, #0d8bd9)"},3e3)}async fillReplyBox(e,t){e.focus(),await new Promise(a=>setTimeout(a,100)),e.tagName==="TEXTAREA"||e.tagName==="INPUT"?e.value="":e.textContent="",e.innerHTML="",e.tagName==="TEXTAREA"||e.tagName==="INPUT"?e.value=t:(e.textContent=t,e.innerHTML=t);const r=new InputEvent("input",{bubbles:!0,inputType:"insertText",data:t});e.dispatchEvent(r);const n=new Event("change",{bubbles:!0});e.dispatchEvent(n),e.focus()}findTweetTextForReply(e){let t=e.closest("article");if(t){const o=t.querySelector("[data-testid=\"tweetText\"]");if(o!=null&&o.textContent)return o.textContent.trim()}const a=e.closest("[role=\"dialog\"]");if(a){const o=a.querySelector("[data-testid=\"tweetText\"]");if(o!=null&&o.textContent)return o.textContent.trim()}const r=document.querySelectorAll("[data-testid=\"tweetText\"]");if(r.length>0){let o=null,c=1/0;const l=e.getBoundingClientRect();Array.from(r).forEach(g=>{if(g instanceof HTMLElement){const S=g.getBoundingClientRect(),T=Math.abs(S.bottom-l.top);T<c&&S.top<l.top&&(c=T,o=g)}}),o!=null&&o.textContent&&o.textContent.trim()}return null}destroy(){this.isContextValid=!1,this.observer&&this.observer.disconnect(),document.querySelectorAll(".yapmate-floating-btn, .yapmate-tone-popup, .yapmate-rewrite-btn").forEach(e=>{e.remove()}),this.injectedButtons.clear()}}var g;if((g=chrome==null?void 0:chrome.runtime)!=null&&g.id){const s=new C;window.addEventListener("beforeunload",()=>{s.destroy()}),chrome.runtime.onConnect.addListener(()=>{console.log("Extension context restored")})}else console.warn("Extension context not available, skipping initialization");